    ...
    <>
      <div className={style.card}>
        ...
        <div className={style.cardInterno}>
              ...
              {listaTamanhos.map((tamanho) =>
                tamanho === tamanhoAtual ? (
                  <button
                    ...
                    onClick={() =>{ setTamanhoAtual(tamanho); setQuantidade(1)}}
                  >
                    {tamanho}
                  </button>
                ) : (
          ...
          <div className={style.botaoQuant}>
            <button className={style.botaoMenos}
              onClick={() => {
                if (quantidade > 1) {
            ...
            <p>{quantidade}</p>
            <button className={style.botaoMais}
              onClick={() => {
          ...
          <div className={style.botoesFinal}>
            <button onClick={()=>carrinho()} className={style.carrinho}>ADICIONAR AO CARRINHO</button>
            <button
            onClick={() => {
                if(pijamaPagina){
      ...
      <div className={style.elementos}>
        {gerarAtributos().map((elemento, index) => (
            <div key={index}>
                <img className = {style.tipo} src={elemento} alt={`Ícone ${index + 1}`} />
            </div>
        ))}
        </div>
    </>
